...
> The `verify_command` field maps directly to devap's `Task.verify_command` or `test_levels[].command`.
...
> This command can be used directly as devap's `verify_command`.
...
**Verify Command**:
```bash
pnpm vitest run src/auth/login.integration.test.ts
```
